...
import (
    ...
    crypto "github.com/yu-org/yu/apps/hotstuff/chainedhotstuff/crypto"
...
func (s *DefaultSaftyRules) CheckVote(qc storage.QuorumCertInterface, logid string, validators []string) error {
	...
	if !isInSlice(signs[0].GetAddress(), validators) {
		logrus.Error("DefaultSaftyRules::CheckVote error", "validators", validators, "from", signs[0].GetAddress())
...
func (s *DefaultSaftyRules) CheckProposal(proposal, parent storage.QuorumCertInterface, justifyValidators []string) error {
	...
	logrus.Debug("DefaultSaftyRules::CheckProposal", "parent", parent, "justifyValidators", justifyValidators)
	validCnt := 0
