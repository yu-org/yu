...
import (
	...
	kv2 "github.com/yu-org/yu/infra/storage/kv"
	"github.com/yu-org/yu/utils/compress"
...
func NewNodeKeeper(cfg *config.NodeKeeperConf) (*NodeKeeper, error) {
	...
	repoDB, err := kv2.NewBolt(repoDbPath)
	if err != nil {
		return nil, err
	}
	workerDB, err := kv2.NewBolt(workerDbPath)
	if err != nil {
		return nil, err
	}
...
// zipFilePath just like: path/to/github.com/yu-org/yuRepo_linux-amd64_3.zip
// 'github.com/yu-org/yuRepo' is the name of repo
// 'linux-amd64' is the arch of repo
// '3' is the version of repo
func (n *NodeKeeper) convertToRepo(zipFilePath, fname string) error {

	// repoVDir: path/to/github.com/yu-org/yuRepo_linux-amd64_3
	...
	// repoName: github.com/yu-org/yuRepo
	repoName := strings.TrimSuffix(fname, "_"+repoArch+"_"+repoVersionStr+CompressedFileType)

	// repoDir: path/to/github.com/yu-org/yuRepo/3/linux-amd64
...
func getWorkerWithTx(tx kv2.KvTxn, ip string) (*WorkerInfo, error) {
...
func setWorkerWithTx(tx kv2.KvTxn, ip string, info *WorkerInfo) error {
	infoByt, err := info.EncodeMasterInfo()
	if err != nil {
		return err
	}
